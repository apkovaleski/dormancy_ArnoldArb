...
ggplot()+
  geom_point(aes(x=LTE, y=sumdeacc, col=Species),shape=21,size=1.5,stroke=0.5,data=dat3b)+
  geom_point(aes(x=LTE, y=sumdeacc, col=Species),size=1.5,stroke=0.3,data=dat3bb)+
  geom_smooth(aes(x=LTE, y=sumdeacc), fullrange=T, color="black",size=0.8, method='lm', formula=y~x, se=F,data=dat3bb )+
  ylab("Predicted cold hardiness (°C)") + xlab("Observed cold hardiness (°C)") + labs(color = "Species") +
  scale_x_continuous(limits=c(-33,0))+
  scale_y_continuous(limits=c(-33,0))+
  geom_abline(slope=1,intercept=0, lty=1, size=0.2)+
  geom_abline(slope=1,intercept=-5, lty=2, size=0.2)+
  geom_abline(slope=1,intercept=5, lty=2, size=0.2)+
